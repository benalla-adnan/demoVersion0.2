...
class TaskController extends Controller
{
    ...
    public function __construct(Request $request, Task $task, ProjectTimesheetDataTable $projectTimesheetDataTable, TaskDataTable $taskDataTable, projects $project, AllTaskDataTable $allTaskDataTable, EmailController $email)
    {
    ...
    public function index($id = null)
    {
        ...
        $data['task_statuses_all'] = TaskStatus::all();
        $taskList = TaskStatus::getAll();
        $tags                    = Tag::pluck('name')->toArray();
        $data['tags']            = json_encode($tags);
        $data['projects']        = projects::all()->where('project_status_id', '!=', 6);
        ...
        //Individual counting
        if ($allassignee != Auth::guard('admin')->user()->id) {
        ...
        return $this->allTaskDataTable->render('backend.pages.task.list', $data);
    }
    ...
    public function taskFileSlice($files)
    {
        ...
        for ($i = 0; $i < count($files); $i++) {
            ...
            if ($files[$i]->customer_id && $files[$i]->customer_id != 0) {
                $customer = customers::find($files[$i]->customer_id);
    ...
    public function mailToAssignee($assignee, $task_id, $attachments = null)
    {
        ...
        $task->start_date = isset($task->start_date) && !empty($task->start_date)? $task->start_date : '-';
        $task->due_date = isset($task->due_date) && !empty($task->due_date)? $task->due_date : '-';
    ...
    public function addTask()
    {
        ...
        return view('backend.pages.task.add', $data);
    }
    ...
    public function taskStore(Request $request)
    {
        ...
        //    return $request;
            ...
            $task['description']    = $request->task_details;
            $task['priority_id']    = $request->priority_id;
            $task['start_date']     = $request->start_date ? $request->start_date : null;
            $task['due_date']       = $request->due_date ? $request->due_date : null;
            $task['user_id']        =  Auth::guard('admin')->user()->id;
            $task['task_status_id'] = 1;

            $related_to_type = $request->related_to;
            $related_to_id = $request->projects;
            $task['related_to_id']       = $related_to_id;
            $task['related_to_type']     = $related_to_type;
            $task['hourly_rate']         = isset($request->hourly_rate) ? $request->hourly_rate : 0;
            $task['milestone_id'] = isset($related_to_type) && $related_to_type == 1 ? $request->milestone_id : null;
            $insertId = DB::table('tasks')->insertGetId($task);
            if ($insertId) {
            ...
            if (isset($request->assignee)) {
                ...
                //If it is a task related to a project
                    $assigns = ProjectMember::where(['project_id'=> $related_to_id])
                            ->whereIn('user_id', $request->assignee)->get();
                    if ($assigns) {
                        foreach ($assigns as $assign) {
                            DB::table('task_assigns')->insert(['task_id' => $insertId, 'user_id' => $assign->user_id, 'assigned_by' => Auth::guard('admin')->user()->id]);
                        }
                    }
                } else {
                    //return 'ok2';
                    foreach ($request->assignee as $value) {
                        DB::table('task_assigns')->insert(['task_id' => $insertId, 'user_id' => $value, 'assigned_by' => Auth::guard('admin')->user()->id]);
                    }
                }
            }
            // return 'hi';
        ...
        //     }
            ...
            return redirect('admin/project/task/view');
        ...
        if (!empty($task)) {
            if (!empty($task->related_to_type)) {
                if ($task->related_to_type == 1 ) {
                    $data['relatedName'] = projects::where('id', $task->related_to_id)->first(['project_name'])->project_name;
                } else if ($task->related_to_type == 2 ) {
                    $data['relatedName'] = customers::where('id', $task->related_to_id)->first(['name'])->name;
                } else {
            ...
            return view('backend.pages.task.edit', $data);
        } else {
            Session::flash('fail', __('The data you are trying to access is not found.'));
            return redirect()->back();
        }
            ...
            if (isset($request->assignee)) {
                ...
                if ( $new_assignee) {
                    foreach ($new_assignee as $value) {
                       DB::table('task_assigns')->insert(['task_id' => $request->task_id, 'user_id' => $value, 'assigned_by' =>  Auth::guard('admin')->user()->id]);
                    }
                }
    ...
    public function tasks_pdf()
    {
        ...
        $data['projectName'] = !empty($project) ? projects::where('id', $project)->first(['name']) : null ;
    ...
    public function projectTaskPdf (Request $request)
    {
        ...
        $data['projectName']  = projects::where('id', $project)->first(['name']);
    ...
    public function relatedSearch(Request $request)
    {
        ...
        if (!empty($request->relatedTo)) {
            if ($request->relatedTo == 1) {
                $result = projects::where('name', 'LIKE', '%' . $request->search . '%')->where('project_status_id', '!=', 6)->get(['id', 'name', 'charge_type'])->take(10);
            } else if ($request->relatedTo == 2) {
                $result = customers::where('name', 'LIKE', '%' . $request->search . '%')->where('is_active', 1)->get(['id', 'name'])->take(10);
            } else {
