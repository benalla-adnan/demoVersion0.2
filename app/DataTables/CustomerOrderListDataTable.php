...
class CustomerOrderListDataTable extends DataTable
{
   ...
   
    {
        ...
        ->addColumn('action', function ($customerOrder) {
            ...
            $edit = $usr->has_permission(Auth::user()->id, 'edit_quotation') ? '<a href="'.url('order/edit/'.$customerOrder->so_id).'" class="btn btn-xs btn-primary"><i class="feather icon-edit"></i></a>&nbsp;':'';

            if($usr->has_permission(Auth::user()->id, 'delete_quotation')){
