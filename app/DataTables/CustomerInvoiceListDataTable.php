...
class CustomerInvoiceListDataTable extends DataTable
{
   ...
   
    {
        ...
        ->addColumn('action', function ($customerInvoice) {
            ...
            $edit = $usr->has_permission(Auth::user()->id, 'edit_invoice') ? '<a href="'. url('invoice/edit/'. $customerInvoice->so_id) .'" class="btn btn-xs btn-primary"><i class="feather icon-edit"></i></a>&nbsp;':'';
            
            if($usr->has_permission(Auth::user()->id, 'delete_invoice')){
