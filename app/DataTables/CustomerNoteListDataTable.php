    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($notes) {
                ...
                $edit = $delete = '';
                $edit = ($usr->has_permission(Auth::user()->id, 'edit_customer_note')) ? '<button class="btn btn-xs btn-primary notecontent" data-toggle="modal" data-target="#editNote" id="edit-customer-note" data-note_id="' . $notes->id . '"><i class="feather icon-edit"></i></button>&nbsp;' : '';

                $delete = ($usr->has_permission(Auth::user()->id, 'delete_customer_note')) ? '
