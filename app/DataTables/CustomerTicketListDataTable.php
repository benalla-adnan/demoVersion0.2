    ...
    public function ajax():JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($tickets) {
                ...
                $edit = ($usr->has_permission(Auth::user()->id, 'edit_ticket')) ? '<a href="' . url("ticket/edit/" . $tickets->id) . '" class="btn btn-xs btn-primary"><i class="feather icon-edit"></i></a>&nbsp;' : '';
                $delete = ($usr->has_permission(Auth::user()->id, 'delete_ticket')) ? '
            ...
            ->addColumn('assignee', function ($tickets) {
                $usr = Auth::guard('admin')->user();
                $assign = '';
                if ($usr->has_permission(Auth::user()->id, 'edit_team_member')) {
