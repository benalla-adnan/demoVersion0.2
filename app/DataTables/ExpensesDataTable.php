...
class ExpensesDataTable extends DataTable
{
    public function ajax():JsonResponse
    {
        ...
        ->addColumn('action', function ($expenses) {
            ...
            $edit = $usr->has_permission(Auth::user()->id, 'edit_expense') ? '<a title="'. __('Edit') .'" href="'.url('expense/edit-expense/'.$expenses->id).'" class="btn btn-xs btn-primary"><i class="feather icon-edit"></i></a>&nbsp;':'';
            if($usr->has_permission(Auth::user()->id, 'delete_expense')){
    ...
    public function query()
    {
        ...
        $expense = Expense::select('expenses.*');
         ...
         if ($usr->has_permission(Auth::user()->id, 'own_expense')) {
            $id = Auth::user()->id;
